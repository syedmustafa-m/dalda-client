...
export default function Idealstatus() {
    ...
    const getAllIdeas = async () => {
        try {
            const { data } = await axios.get(
                `http://10.0.0.200:75/api/ideas/idea-user/${auth.user.userid}`
            );
            setIdeas(data.ideas);
    ...
    const getAllUserIdeas = async () => {
        try {
            const { data } = await axios.get(
                `http://10.0.0.200:75/api/auth/users-managed/${parseInt(
                    auth.user.userid
                )}`
            );
            ...
            for (let i = 0; i < users.length; i++) {
                const { data } = await axios.get(
                    `http://10.0.0.200:75/api/ideas/idea-user/${users[i].userid}`
                );
